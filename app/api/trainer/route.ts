...
async function synthesizePlanBlock(
  ...
  const systemInstruction =
    ...
    '"sets":[{"type":"WARMUP","weight":135,"reps":8},{"type":"WORKING","weight":225,"reps":5}]}]}\n' +
    "```\n\n" +
    "Rules:\n" +
    "- One entry per WORKING set (3×5 at 225 = three set entries with the same weight/reps).\n" +
    "- Preserve every WARMUP set the coach already put in their plan. Additionally, if the session's MAIN compound barbell lift (the first heavy compound — bench, squat, deadlift, overhead press, barbell row, front squat, RDL) has NO warm-up set, prepend one ramp-up WARMUP set (~50% of that lift's working weight, ~8 reps) before its working sets so the athlete's log carries a real warm-up. Only the main lift gets it — never the accessories. Skip entirely for isolation, machine, bodyweight, and deload work.\n" +
...
4. WORKOUT-PLAN APPENDIX (CRITICAL — emit on EVERY workout-prescription reply):
   ...
   - Warm-up sets (type:"WARMUP") belong in the plan block even though the prose stays tight — they get pushed to the athlete's log to check off, NOT described in text. On the session's MAIN compound barbell lift (the first heavy compound — bench, squat, deadlift, overhead press, barbell row, front squat, RDL), prepend ONE ramp-up WARMUP set before its working sets: a lighter load (~40–60% of the working weight) for ~6–8 reps. As in the example above, Barbell Bench Press leads with a {"type":"WARMUP","weight":135,"reps":8} before its working sets. Only the main lift gets a warm-up set — the accessories do not. Skip it entirely for isolation/pump work, machines, deloads, and bodyweight movements. Do NOT add any warm-up prose; the set lives only in the plan block.
   ...
   - MODIFYING AN EXISTING PLAN: when the athlete asks to change a workout you already prescribed — "swap squats for leg press", "replace the bench with incline", "drop the curls", "remove the last exercise", "add a calf raise", "make it 4 sets", "bump the bench to 235", "reorder so deadlifts come first" — you MUST re-emit the COMPLETE updated workout-plan block: every exercise that remains, with the requested change applied, in the new order, NOT just the changed exercise and NOT a prose-only description. Carry over the untouched exercises exactly as they were (same names, sets, reps, weights, restSeconds). Keep the prose to a one-line confirmation of what changed ("Swapped in leg press — here's the updated session."). Apply the same plan-block rules above (warm-up set on the main lift, one entry per working set, integer reps, valid restSeconds). This re-renders the "Do this workout" button with the new plan, so the athlete can freely shape the session by chatting.
