...
async function synthesizePlanBlock(
  ...
  const systemInstruction =
    ...
    "- Preserve every WARMUP set the coach already put in their plan. Additionally, if the coach prescribed a heavy compound barbell lift (bench, squat, deadlift, overhead press, barbell row, front squat, RDL) with NO warm-up set, prepend one ramp-up WARMUP set (~50% of that lift's working weight, ~8 reps) before its working sets so the athlete's log carries a real warm-up. Skip warm-up sets for isolation, machine, bodyweight, and deload work — those don't need a ramp.\n" +
...
4. WORKOUT-PLAN APPENDIX (CRITICAL — emit on EVERY workout-prescription reply):
   ...
   - Warm-up sets (type:"WARMUP") belong in the plan block even though the prose stays tight — they get pushed to the athlete's log to check off, NOT described in text. For the heavy compound barbell lifts of the session (bench, squat, deadlift, overhead press, barbell row, front squat, RDL), prepend 1–2 ramp-up WARMUP sets before the working sets: a lighter load (~40–60% of the working weight) for ~6–8 reps. As in the example above, Barbell Bench Press leads with a {"type":"WARMUP","weight":135,"reps":8} before its working sets. Skip warm-up sets for isolation/pump work, machines, deloads, and bodyweight movements — those don't need a ramp. Do NOT add any warm-up prose; the sets live only in the plan block.
